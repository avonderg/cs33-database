...
server_control_t server = {PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER,
                           0};
client_control_t client = {PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER,
                           0};

/**
 * Called by client threads to wait until progress is permitted. Blocks the
 * calling thread until the main thread calls client_control_release.
 * **/
void client_control_wait() {
    pthread_mutex_lock(&client.go_mutex);
    pthread_cleanup_push(
        (void (*)(void *))pthread_mutex_unlock,
        &client.go_mutex);    // cancellation point (CAST TO VOID???)
...
/**
 * Called by listener (in comm.c) to create a new client thread. Creates a new
 * client_t struct and initializes all of its fields.
 *
 * Parameters:
...
void client_constructor(FILE *cxstr) {
    ...
    newClient->prev = NULL;
    int err1;
    if ((err1 = pthread_create(&newClient->thread, 0, run_client, newClient)) !=
        0) {  // creates a thread
        handle_error_en(err1, "pthread_create");
    }
    int err2;
    if ((err2 = pthread_detach(newClient->thread)) !=
        0) {  // detaches the new client thread
...
/**
 * Code executed by a client thread. Adds a client to the client list, and
 * pushes thread_cleanup to remove it if the thread is cancelled. Loops
 * comm.serve to receive commands and output responses, and executes commands.
 *
 * Parameters:
...
void *run_client(void *arg) {
    ...
    if (accepted == 1) {  // if clients are still being accepted
        ...
        if (thread_list_head ==
            NULL) {  // if the head of the list is null, insert
        ...
        pthread_mutex_lock(
            &server.server_mutex);  // lock to increment # clients
        ...
        while (comm_serve(client->cxstr, response, command) ==
            ...
            client_control_wait();  // makes sure running client (if stop =
                                    // true), ie, makes sure threads wait
...
/**
 * Cancels every thread in the client thread list with the pthread_cancel
 * function.
 * **/
void delete_all() {
    client_t *curr = thread_list_head;
...
/**
 * Cleanup routine for client threads, called on cancels and exit. Removes the
 * client object from the thread list and calls client_destructor.
 *
 * Parameters:
...
void thread_cleanup(void *arg) {
    ...
    if (client == thread_list_head) {  // if the client is at the head of list
        ...
        if (client->next != NULL) {  // if it was not the only item in the list
            client->next->prev = NULL;
               ...
               NULL) {  // if the client is at the end of the list
    ...
    if (server.num_client_threads == 0) {
        pthread_cond_signal(
            &server.server_cond);  // ensures it is safe to call cleanup
    }
    pthread_mutex_unlock(&server.server_mutex);
    return;
}
...
/**
 * Code executed by the signal handler thread. Waits for a SIGINT to be sent to
 * the server process and cancels all cilent threads when one arrives.
 *
 * Parameters:
 * - arg: a void pointer to the client_t client
 *
 ...
 *
 * Returns:
 * - sig: the sighandler_t struct created containing the thread responding to
 * SIGINT
 * **/
sig_handler_t *sig_handler_constructor() {
    ...
    int error1;
    if ((error1 = pthread_sigmask(SIG_BLOCK, &sig->set, 0)) !=
        0) {  // blocks SIGINT
        handle_error_en(error1, "pthread_sigmask");
    }
    int error2;
    if ((error2 = pthread_create(&sig->thread, 0, monitor_signal, &sig->set)) !=
        0) {  // creates thread
...
/**
 * Frees any resources allocates in sig_handler_constructor. Cancels and joins
 * with the signal handler's thread.
 *
 * Parameters:
...
int main(int argc, char *argv[]) {
    ...
    int error5;
    if ((error5 = pthread_sigmask(SIG_BLOCK, &signal, NULL)) !=
        0) {  // blocks SIGPIPE
    ...
    while (1) {  // creates REPL
        ...
        if (to_read == -1) {
            ...
            delete_all();  // sends cancellation req to all clients, wait for
                           // last thread to finish destroying
            pthread_mutex_unlock(&thread_list_mutex);
            pthread_mutex_lock(&server.server_mutex);
